...
public class SegmentsExperienceUpgradeProcess extends UpgradeProcess {
	...
	private void _upgradeDefaults() throws Exception {
		try (PreparedStatement updatePreparedStatement =
				connection.prepareStatement(
					StringBundler.concat(
						"update SegmentsExperience set segmentsEntryERC = ?, ",
						"segmentsEntryScopeERC = ? where segmentsEntryId = ? ",
						"and segmentsExperienceKey = ?"))) {

			updatePreparedStatement.setString(
				1, SegmentsEntryConstants.KEY_DEFAULT);
			updatePreparedStatement.setString(2, null);
			updatePreparedStatement.setLong(
				3, SegmentsEntryConstants.ID_DEFAULT);
			updatePreparedStatement.setString(
	...
	private void _upgradeSegmentsEntryLinks() throws Exception {
