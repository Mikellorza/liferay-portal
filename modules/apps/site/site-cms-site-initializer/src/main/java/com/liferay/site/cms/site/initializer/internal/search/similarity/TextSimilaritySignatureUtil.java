...
public class TextSimilaritySignatureUtil {
	...
	private static long[] _getMinHashSignature(Set<String> shingles) {
		...
		for (String shingle : shingles) {
			...
			for (int i = 0; i < _HASH_COUNT; i++) {
				long permutedHash = _getPermutedHash(baseHash, i);
	...
	private static long _getNextSeed(long seed) {
		return (seed * 6364136223846793005L) + 1442695040888963407L;
	}

	private static long _getPermutedHash(long hash, int index) {
		long permutedHash =
			((_PERMUTATION_A[index] * hash) + _PERMUTATION_B[index]) %
				_MERSENNE_PRIME;

		if (permutedHash < 0) {
			permutedHash += _MERSENNE_PRIME;
		}
	...
	static {
		...
		for (int i = 0; i < _HASH_COUNT; i++) {
			seed = _getNextSeed(seed);

			_PERMUTATION_A[i] = ((seed >>> 1) % (_MERSENNE_PRIME - 1)) + 1;

			seed = _getNextSeed(seed);

			_PERMUTATION_B[i] = (seed >>> 1) % _MERSENNE_PRIME;
		}
	}
