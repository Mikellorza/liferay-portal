...
public class SimilarAssetUtil {
	...
	public static String[] getSimilarAssetBuckets(String text) {
		if (text == null) {
			return new String[0];
		}
		...
		String[] similarAssetBuckets = new String[_SIMILAR_ASSET_BUCKETS];

		for (int i = 0; i < _SIMILAR_ASSET_BUCKETS; i++) {
			StringBundler sb = new StringBundler(
				(_SAMPLES_PER_SIMILAR_ASSET_BUCKET * 2) + 2);

			sb.append("b");
			sb.append(i);

			for (int j = 0; j < _SAMPLES_PER_SIMILAR_ASSET_BUCKET; j++) {
				sb.append(StringPool.UNDERLINE);
				sb.append(
					_similarityHashPolicy.getComponent(
						similarityHash,
						(i * _SAMPLES_PER_SIMILAR_ASSET_BUCKET) + j));
			}

			similarAssetBuckets[i] = sb.toString();
		}

		return similarAssetBuckets;
	}
	...
	private static final int _SAMPLES_PER_SIMILAR_ASSET_BUCKET = 4;

	private static final int _SIMILAR_ASSET_BUCKETS = 32;
	...
	private static final SimilarityHashPolicy _similarityHashPolicy =
		SimilarityHashing.superMinHash(
			_SIMILAR_ASSET_BUCKETS * _SAMPLES_PER_SIMILAR_ASSET_BUCKET,
			_BITS_PER_SAMPLE, SuperMinHashVersion.V1);
