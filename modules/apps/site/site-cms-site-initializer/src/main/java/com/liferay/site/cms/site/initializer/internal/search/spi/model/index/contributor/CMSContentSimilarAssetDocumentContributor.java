...
public class CMSContentSimilarAssetDocumentContributor
	...
	public void contribute(
		...
		try {
			...
			List<String> similarAssetBuckets = new ArrayList<>();

			for (String languageId :
					_cmsContentSimilarAssetTextExtractor.getLanguageIds(
						objectEntry)) {

				similarAssetBuckets.addAll(
					TransformUtil.transformToList(
						SimilarAssetUtil.getSimilarAssetBuckets(
							_cmsContentSimilarAssetTextExtractor.getText(
								languageId, objectEntry)),
						similarAssetBucket -> StringBundler.concat(
							languageId, StringPool.UNDERLINE,
							similarAssetBucket)));
			}

			if (similarAssetBuckets.isEmpty()) {
				return;
			}

			document.addKeyword(
				"similarAssetBuckets",
				similarAssetBuckets.toArray(new String[0]));
		}
		catch (Exception exception) {
			if (_log.isWarnEnabled()) {
				_log.warn(
					"Unable to contribute similar asset buckets for object " +
